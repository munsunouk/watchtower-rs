...
use crate::utils::{
    constants::{ChainID, DEFAULT_CALL_RETRIES, DEFAULT_CALL_RETRY_INTERVAL_MS},
    error::{INVALID_CHAIN_ID, PROVIDER_INTERNAL_ERROR},
};
use anyhow::Result;
use ethers::{
    abi::Detokenize,
...
impl<T: JsonRpcClient> EthClient<T> {
    ...
    /// until it exceeds the maximum retries.
    pub async fn contract_call<M, D>(&self, raw_call: ContractCall<M, D>, method: &str) -> Result<D>
    where
        M: Middleware,
    ...
    pub async fn verify_chain_id(&self) -> Result<()> {
        ...
        if self.get_chain_id() != chain_id.as_u32() {
            tracing::error!("[{}] ❗️ [{}]", &self.get_chain_name(), INVALID_CHAIN_ID);
    ...
    pub async fn get_block_with_txs(&self, id: BlockId) -> Result<Option<Block<Transaction>>> {
        self.rpc_call("eth_getBlockByNumber", (id, true)).await
    }

    /// Retrieves the block information of the given block hash.
    pub async fn get_block(&self, id: BlockId) -> Result<Option<Block<H256>>> {
    ...
    pub async fn is_syncing(&self) -> Result<SyncingStatus> {
